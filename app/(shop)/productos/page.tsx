...
export const metadata: Metadata = {
  title: "Catálogo",
  description:
    "Explora nuestro catálogo de bisutería y accesorios elegantes. Filtra por categoría, precio y más.",
}
...
export default async function CatalogPage({
  ...
  if (categorySlug) {
    const { data: catRaw } = await s
      .from("categories")
      .select("id")
      .eq("slug", categorySlug)
      .single()
    const cat = catRaw as { id: number } | null
    if (cat) {
  ...
  const buildQueryString = (overrides: Record<string, string>) => {
    ...
    if (sort && sort !== "newest") current.sort = sort
    if (q) current.q = q
                ...
                <div className="flex justify-center gap-2 mt-10">
                  ...
                  {page < totalPages && (
                    <>
                      ...
                      <Link
                        href={buildQueryString({ page: String(totalPages) })}
                        className="px-4.5 py-2.5 rounded-[8px] no-underline text-[14px] font-medium transition-all duration-300 bg-white text-[#4A4A4A] border border-[#EEE] hover:bg-primary hover:text-white hover:border-primary"
                        prefetch={false}
                      >
                        &raquo;
